...
//console.log(get_path("BiometricPhp"))
...
function activeSensorRead(showMessage) {
    if (!localStorage.getItem("srnPc")) {
        ...
        jQuery.ajax({
            type: 'POST',
            url: get_path("BiometricPhp") + "/web/active_sensor_read",
            data: data,
            dataType: 'json',
            contentType: false,
            processData: false,
        ...
        inputValidator: (value) => {
            return new Promise((resolve) => {
                if (value !== 'Seleccione') {
                    ...
                    var data = new FormData();
                    data.append("token_pc", localStorage.getItem("srnPc"));
                    data.append("user_id", jQuery(this).data('id'));
                    data.append("finger_name", value);
                    jQuery.ajax({
                        type: 'POST',
                        url: get_path("BiometricPhp") + "/web/active_sensor_enroll",
                        data: data,
                        dataType: 'json',
                        contentType: false,
                        processData: false,
...
function getFingerprintByUser() {
    ...
    var _url = get_path("BiometricPhp") + "/Views/finger-list.php/" + userId;
    jQuery.get(get_path("BiometricPhp") + "/web/get-finger/" + userId, function (data) {
        if (data.length > 0) {
...
function getData() {
    if (typeof (EventSource) !== 'undefined') {
        var source = new EventSource(get_path("BiometricPhp") + '/web/ssejs/' + localStorage.getItem("srnPc"));
